 ...
 const [followed, setFollowed] = useState([currentUser.followings.includes(user?._id)]);
  ...
  ProfileRightbar=()=>{
    ...
    return(
      ...
      {(user.occupation || user.relationship || user.city) && (<h4 className="profileRightbarTitle">User Information</h4> )}
      ...
      <div className="rightbarInfo">
        ...
        <div className="rightbarInfoItem">
         ...
         {user.city && ( <span className="rightbarInfoValue">{user.city}</span>)}
        </div>
        <div className="rightbarInfoItem">
          {user.occupation && (<span className="rightbarInfoKey">Occupation : </span>)}
          {user.occupation && (<span className="rightbarInfoValue">{user.occupation}</span>)}
        </div>
        <div className="rightbarInfoItem">
        {user.relationship && (<span className="rightbarInfoKey">Status : </span>)}
        {user.relationship && (<span className="rightbarInfoValue">{user.relationship}</span>)}
        </div>
      </div> 
